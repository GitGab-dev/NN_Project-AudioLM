...
class RelativePosition(nn.Module):
    def __init__(self, d_a: int, k: int, myDevice: torch.device):
        ...
        self.position_embeddings = nn.Parameter(
            torch.empty((2 * k + 1, d_a), device=self.myDevice)
        )
...
class AttentionHead(nn.Module):
    def __init__(self, hidden_size, d_model, myDevice: torch.device):
        ...
        self.myDevice = myDevice
        self.query_weights: nn.Linear = nn.Linear(hidden_size, self.d_model).to(
            myDevice
        )
        self.key_weights: nn.Linear = nn.Linear(hidden_size, self.d_model).to(myDevice)
        self.value_weights: nn.Linear = nn.Linear(hidden_size, self.d_model).to(
            myDevice
        )

    def forward(
        self,
        queryIn: torch.Tensor,
        keyIn: torch.Tensor,
        valueIn: torch.Tensor,
        maskIn: torch.Tensor = None,
        k_bias_matrix: torch.Tensor = None,
        v_bias_matrix: torch.Tensor = None,
    ) -> torch.Tensor:
        ...
        attn_2: torch.Tensor = torch.matmul(
            query.transpose(0, 1), k_bias_matrix.transpose(1, 2)
        ).transpose(0, 1)
        ...
        values_2: torch.Tensor = torch.matmul(
            att_weights.transpose(0, 1), v_bias_matrix
        ).transpose(0, 1)
...
class MultiHeadAttention(nn.Module):
    def __init__(
        ...
        k=64,
        seq_len=1500,
        myDevice: torch.device = torch.device("cpu"),
    ):
        super().__init__()
        ...
        self.attention_heads = nn.ModuleList(
            [
                AttentionHead(self.hidden_size, self.head_dim, myDevice)
    ...
    def forward(
        ...
        attention_outputs: List[torch.Tensor] = [
            attention_head(query, key, value, mask, k_bias_matrix, v_bias_matrix)
...
class Decoder(pl.LightningModule):
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [
                DecoderLayer(
                    d_model, num_heads, dim_feedforward, dropout, k, seq_len, myDevice
                )
        ...
        # Causal mask
        self.causal_mask = (
            torch.tril(torch.ones((seq_len, seq_len), device=myDevice))
            .unsqueeze(0)
            .detach()
        )
    ...
    def on_train_epoch_end(self):
        torch.save(
            self.state_dict(),
            f"checkpoints/{self.__class__.__name__}_epoch={self.current_epoch:02d}.ckpt",
        )
            ...
            with torch.no_grad():
                if padding_mask is not None:
                    ...
                    padding_mask = torch.cat(
                        [
                            padding_mask[:, :, 1:],
                            torch.tensor([[[1]]], device=self.myDevice),
                        ],
                        dim=2,
                    )
                else:
                ...
                first_free_spot = (padding_mask[0, 0, cut_value:] == 0).nonzero(
                    as_tuple=True
                )
                if len(first_free_spot[0]) > 0:
                    ...
                    total_sequence = torch.cat(
                        (total_sequence, next_token.unsqueeze(0)), dim=1
                    )

                generated_sequence = torch.cat(
                    (generated_sequence[:, 1:], next_token.unsqueeze(0)), dim=1
                )
    ...
    def pad_sequence(self, sequence, length):
        ...
        if to_pad > 0:
            ...
            raise ValueError(
                f"Invalid token length, shorten the input to match this size {length}"
            )
...
class SemanticTransformer(Decoder):
    ...
    def generate_tokens(self, semantic_tokens, num_tokens=None):
        ...
        padded_semantic_tokens, padding_mask = self.pad_sequence(
            semantic_tokens, self.seq_len
        )
    ...
    def common_step(self, batch):
        ...
        combined_mask = self.causal_mask * (input_ids != 0).float().unsqueeze(1).expand(
            -1, self.seq_len, self.seq_len
        )
        output = self(input_ids, combined_mask)

        _, input_seq_len = input_ids.shape
        target_seq_len = target_ids.shape[1]
        output_target = output[:, input_seq_len - target_seq_len :, :]
...
class CoarseTransformer(Decoder):
    ...
    def generate_tokens(self, semantic_tokens, coarse_tokens, num_tokens=None):
        ...
        padded_semantic_tokens, semantic_padding = self.pad_sequence(
            semantic_tokens, self.semantic_size
        )
        padded_coarse_tokens, coarse_padding = self.pad_sequence(
            coarse_tokens, self.coarse_size
        )
        sequence = torch.cat((padded_semantic_tokens, padded_coarse_tokens), dim=0).to(
            self.myDevice
        )

        if semantic_padding is not None and coarse_padding is not None:
            padding_mask = torch.cat((semantic_padding, coarse_padding), dim=1).to(
                self.myDevice
            )
        elif semantic_padding is not None:
            padding_needed = self.seq_len - semantic_padding.shape[1]
            padding_mask = torch.cat(
                [semantic_padding, torch.ones(1, padding_needed, device=self.myDevice)],
                dim=1,
            )
        elif coarse_padding is not None:
            padding_needed = self.seq_len - coarse_padding.shape[1]
            padding_mask = torch.cat(
                [torch.ones(1, padding_needed, device=self.myDevice), coarse_padding],
                dim=1,
            )
        else:
            padding_mask = None

        total_token_sequence = super().generate_tokens(
            sequence, padding_mask, num_tokens, self.semantic_size
        )
    ...
    def common_step(self, batch):
        ...
        ]  # I take only the tokens I need to check (for the coarse generation I take only the coarse generated)

        # Reshape for CrossEntropyLoss
        output_target = output_target.reshape(
            -1, output_target.size(-1)
...
class FineTransformer(Decoder):
    ...
    def generate_tokens(self, coarse_tokens, fine_tokens, num_tokens=None):
        ...
        padded_fine_tokens, fine_padding = self.pad_sequence(
            fine_tokens, self.fine_size
        )
        sequence = torch.cat((padded_coarse_tokens, padded_fine_tokens), dim=0).to(
            self.myDevice
        )

        if fine_padding is not None and coarse_padding is not None:
            padding_mask = torch.cat((coarse_padding, fine_padding), dim=1).to(
                self.myDevice
            )
        elif coarse_padding is not None:
            padding_needed = self.seq_len - coarse_padding.shape[1]
            padding_mask = torch.cat(
                [coarse_padding, torch.ones(1, padding_needed, device=self.myDevice)],
                dim=1,
            )
        elif fine_padding is not None:
            padding_needed = self.seq_len - fine_padding.shape[1]
            padding_mask = torch.cat(
                [torch.ones(1, padding_needed, device=self.myDevice), fine_padding],
                dim=1,
            )
        else:
            padding_mask = None
        ...
        total_token_sequence = super().generate_tokens(
            sequence, padding_mask, num_tokens, self.coarse_size
        )
...
def generate_new_sequence(
    ...
    """generate an audio extension

    Args:
        semantic_tokens (TokenList): semantic tokens of the input
        coarse_tokens (TokenList): coarse tokens of the input
        fine_tokens (TokenList): fine tokens of the input
        semantic_model (SemanticTransformer): semantic transformer instance
        coarse_model (CoarseTransformer): coarse transformer instance
        fine_model (FineTransformer): fine transformer instance
        audioDuration (int): desired audio lenght in seconds
        Q (int, optional): number of total quantizers. Defaults to 8.
        Q_prime (int, optional): number of quantizers reserved to coarse tokens. Defaults to 3.

    Returns:
        Tuple(TokenList,TokenList): generated coarse and fine tokens (input included).
    """
    ...
    with torch.no_grad():
        ...
        if semantic_to_generate > 0:
            ...
            new_semantic = semantic_model.generate_tokens(
                current_semantic, semantic_to_generate
            ).squeeze(0)
            total_semantic = torch.cat(
                [semantic_tokens[: i * max_sem], new_semantic], dim=0
            )
        else:
        ...
        if coarse_to_generate > 0:
            ...
            while coarse_to_generate > 0:
                new_coarse = coarse_model.generate_tokens(
                    current_semantic, current_coarse
                ).squeeze(0)
                generated = new_coarse.shape[0] - current_coarse.shape[0]
                coarse_to_generate = coarse_to_generate - generated
                coarse_tokens = torch.cat(
                    [coarse_tokens, new_coarse[current_coarse.shape[0] :]], dim=0
                )
                current_coarse = coarse_tokens[-max_coarse // 2 :]
                i = i + 1
                if i * max_sem // 2 < total_semantic.shape[0]:
                    current_semantic = total_semantic[
                        i * max_sem // 2 : (i + 1) * max_sem // 2
                    ]
                else:
                    break
        ...
        if fine_to_generate > 0:
            ...
            while fine_to_generate > 0:
                new_fine = fine_model.generate_tokens(
                ...
                fine_tokens = torch.cat(
                    [fine_tokens, new_fine[current_fine.shape[0] :]], dim=0
                )
                current_fine = new_fine[-max_fine // 2 :]
                i = i + 1
                if i * max_coarse // 2 < coarse_tokens.shape[0]:
                    current_coarse = coarse_tokens[
                        i * max_coarse // 2 : (i + 1) * max_coarse // 2
                    ]
                else:
                    break
...
def createSingleModel(
    ...
    Args:
        type (str): decoder type. Can be 'semantic', 'coarse' or 'fine'.
        checkpoint_path (Path, optional): path to .ckpt file. Defaults to None.
        d_model (int, optional): embedding dimension. Defaults to 1024.
        num_layers (int, optional): number of decoder layers. Defaults to 12.
        num_heads (int, optional): number of attention heads. Defaults to 16.
        dim_feedforward (int, optional): feedforward dimension. Defaults to 4096.
        dropout (float, optional): dropout regularization value. Defaults to 0.1.
        audioDuration (int, optional): input audio duration in seconds. Defaults to 3.
        ...
        learning_rate (float, optional): learning rate value. Defaults to 10e-4.
        myDevice (TorchDevice, optional): device on where to set the model. Defaults to torch.device("cpu").

    Raises:
        ValueError: invalid model type

    Returns:
        DecoderModel: initialized decoder model instance
    """
    ...
    if type not in model_classes:
        raise ValueError(
            f"Invalid model type: {type}.\nChoose from 'semantic', 'coarse', or 'fine'."
        )

    total_vocab_size = {"semantic": 500, "coarse": 1024, "fine": 8192}
    ...
    if os.path.exists(checkpoint_path):
        ...
        model = model_class.load_from_checkpoint(
            checkpoint_path, **common_params, **model_extra_params
        )
    else:
...
def createAllModels(
    ...
    dim_feedforward=4096,
    dropout=0.1,
    audioDuration=[30, 10, 3],
    Q_prime=3,
    Q=8,
    vocab_size=[500, 1024, 8192],
    learning_rate=10e-4,
    myDevice=torch.device("cpu"),
):
    ...
    if len(audioDuration) < 3:
        raise ValueError(
            f"Invalid audioDuration format: {len(audioDuration)}.\nWrite a three values array."
        )
    if len(vocab_size) < 3:
        raise ValueError(
            f"Invalid vocabSize format: {len(vocab_size)}.\nWrite a three values array."
        )
    ...
    semanticModel = createSingleModel(
        type="semantic", checkpoint_path=semantic_checkpoint, **params
    )

    params["audioDuration"] = audioDuration[1]
    params["vocab_size"] = vocab_size[1]

    coarseModel = createSingleModel(
        type="coarse", checkpoint_path=coarse_checkpoint, **params
    )

    params["audioDuration"] = audioDuration[2]
    params["vocab_size"] = vocab_size[2]

    fineModel = createSingleModel(
        type="fine", checkpoint_path=fine_checkpoint, **params
    )
